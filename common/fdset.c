...
static void fdset_push(struct fdset_s* self, sock fd)
{
    if(NULL == self->fds)
    {
        self->fds = array_new(fd, sizeof(bool));
    }

    int fd_num = array_num(self->fds);
    if(fd_num <= fd)
    {
        array_increase(self->fds, fd_num);
    }

    bool data = true;
    array_set(self->fds, fd, &data);
...
static void fdset_erase(struct fdset_s* self, sock fd)
{
    bool data = false;
    array_set(self->fds, fd, &data);
