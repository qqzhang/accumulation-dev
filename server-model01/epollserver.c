...
#ifdef PLATFORM_LINUX
static void epoll_add_event(struct epollserver_reactor_s* reactor, sock fd, struct session_s* client, uint32_t events)
{
...
static void epoll_freelist_lock(struct epollserver_s* epollserver)
{
...
static void epoll_freelist_unlock(struct epollserver_s* epollserver)
{
    if(EPOLL_NET_USE_SPINLOCK_OPEN)
    {
...
static void session_send_lock(struct session_s* session)
{
    if(EPOLL_NET_USE_SPINLOCK_OPEN)
    {
...
static void session_send_unlock(struct session_s* session)
{
    if(EPOLL_NET_USE_SPINLOCK_OPEN)
    {
